...
!   Calculation cases are defined in the input file Nemoh.cal which must be located in the working folder.
!   Once the preProcessor has been run successfully, the following files are created in the working folder:
!   /Mesh/L12.dat, /Mesh/L10.dat and /Mesh/Mesh.tec files: They are the mesh files.
!   The first one contains tables of nodes and connectivities. The second one contains additional geometrical
!   information such as use of a symmetry about the (xOz) plane, number of nodes, number of panels,
!   number of bodies, panel to body belonging, panel centres, panel normal vectors and panel areas.
!   The last one is in Tecplot compatible format for vizualisation purpose.
!
!   /Normalvelocities.dat file: This file contains the body conditions for each radiation and diffraction
!   problem defined in the Nemoh.cal file.
!
!   /Integration.dat file: This file contains the information about how the pressure has to be integrated
!   over the body surfaces to obtain the requested forces (as defined in the Nemoh.cal file).
!
!   /Results/FKForce.dat, /Results/FKForce.tec files: Froude Krylov forces for each of the diffraction problem.
!   Format of the second file is compatible with Tecplot. It may be used for vizualisation of
!   Froude Krylov forces.
!
!   /Results/FreeSurface.dat file: This file contains the coordinates of the free surface where to
...
!
!   /Results/index.dat file: This table gives correlatoin between force ID number,
!   body ID number on which it applies and number if the Nemoh.cal file.
!   It also contains the correlation between ID number of radiation problem, ID of body
!   which is actually moving and the number of degree of freedom.
!
!   @author TCSASSEMBLER
!   @version 1.1
PROGRAM Main
    ...
    !   Read number of radiation cases, number of force integration cases,
    !   range of considered periods and diffraction cases
    ...
    DO c=1,Mesh%Nbodies
        ...
        READ(10,*) M
        DO i=1,M
            READ(10,*) RadCase(jrad+i)%ICase,(RadCase(jrad+i)%Direction(j),j=1,3),&
                &(RadCase(jrad+i)%Axis(j),j=1,3)
        ...
        READ(10,*) M
        DO i=1,M
            READ(10,*) IntCase(jint+i)%ICase,(IntCase(jint+i)%Direction(j),j=1,3),&
                &(IntCase(jint+i)%Axis(j),j=1,3)
    ...
    DO i=1,Nw
        ...
        END DO
        DO j=1,Nradiation
            CALL ComputeRadiationCondition(Mesh,RadCase(j)%Body,RadCase(j)%Icase,&
                &RadCase(j)%Direction,RadCase(j)%Axis,NVEL)
            DO c=1,Mesh%Npanels*2**Mesh%Isym
    ...
    DO c=1,Mesh%Npanels*2**Mesh%Isym
        WRITE(11,*) (REAL(NormalVelocity(c,j)),AIMAG(NormalVelocity(c,j)),j=1,(Nbeta+Nradiation)*Nw)
    END DO
    CLOSE(11)        
    ...
    DO c=1,Nbeta
        ...
        DO i=1,Nw
            WRITE(10,'(80(1X,E14.7))') w(i),(ABS(FKForce(i,c,k)),&
                &ATAN2(AIMAG(FKForce(i,c,k)),REAL(FKForce(i,c,k))),k=1,Nintegration)
        END DO
    END DO
    CLOSE(10)
    OPEN(10,FILE=ID%ID(1:ID%lID)//'/results/FKForce.dat')
    DO k=1,Nintegration
        WRITE(10,*) ((ABS(FKForce(i,c,k)),ATAN2(AIMAG(FKForce(i,c,k)),&
            &REAL(FKForce(i,c,k))),c=1,Nbeta),(0.*c,0.*c,c=1,Nradiation),i=1,Nw)
    END DO
    CLOSE(10)
    DEALLOCATE(FKForce)
    !
    ...
    DO i=1,Nx
        DO j=1,Ny
            WRITE(11,'(3(1X,E14.7))') -0.5*Lx+Lx*(i-1)/(Nx-1),-0.5*Ly+Ly*(j-1)/(Ny-1),0.
        END DO
    END DO  
    DO i=1,Nx-1
        DO j=1,Ny-1
            WRITE(11,'(4(1X,I7))') j+(i-1)*Ny,j+1+(i-1)*Ny,j+1+i*Ny,j+i*Ny
        END DO
    END DO 
    CLOSE(11)
    !
