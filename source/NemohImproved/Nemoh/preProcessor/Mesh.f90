...
! This module contains the structure to hold a Mesh as well as utilities to read and create it.
!
!   @author TCSASSEMBLER
!   @version 1.1
MODULE PMMesh
...
CONTAINS
    ...
    SUBROUTINE ReadTMesh(Mesh,ID)
        ...
        DO i=1,Mesh%Npanels
            ...
            END IF 
            DO j=1,3
                Mesh%XM(j,i)=1./3*(Mesh%X(j,Mesh%P(1,i))+Mesh%X(j,Mesh%P(2,i))+&
                    &Mesh%X(j,Mesh%P(4,i)))*A1/Mesh%A(i)+1./3*(Mesh%X(j,Mesh%P(2,i))+&
                    &Mesh%X(j,Mesh%P(3,i))+Mesh%X(j,Mesh%P(4,i)))*A2/Mesh%A(i)
            END DO
            U=W1+W2
            DO j=1,3
        ...
        !call execute_command_line('mkdir '//ID%ID(1:ID%lID)//'/mesh')
        !M=SYSTEM('mkdir '//ID%ID(1:ID%lID)//'/mesh')
        OPEN(10,FILE=ID%ID(1:ID%lID)//'/Mesh/L12.dat')
        WRITE(10,'(I3,1X,I3)') 2,Mesh%Isym
        DO i=1,Mesh%Npoints
            WRITE(10,'(I6,3(1X,E14.7))') i,(Mesh%X(j,i),j=1,3)
        END DO
        WRITE(10,'(I6,3(1X,E14.7))') 0,0.,0.,0.
        DO i=1,Mesh%Npanels
            WRITE(10,'(4(1X,I6))') (Mesh%P(j,i),j=1,4)
        END DO
        WRITE(10,'(4(1X,I6))') 0,0,0,0
        CLOSE(10)
        OPEN(10,FILE=ID%ID(1:ID%lID)//'/Mesh/L10.dat')
        WRITE(10,*) 
        WRITE(10,'(4(1X,I6))') Mesh%Isym,Mesh%Npoints,Mesh%Npanels,Mesh%Nbodies
        DO i=1,Mesh%Npanels
            WRITE(10,'(I5,7(1X,E14.7))') Mesh%cPanel(i),(Mesh%XM(j,i),j=1,3),(Mesh%N(j,i),j=1,3),Mesh%A(i)
        END DO
        WRITE(10,*) 0
        CLOSE(10)
    ...
    !call execute_command_line('mkdir '//ID%ID(1:ID%lID)//'/results')
