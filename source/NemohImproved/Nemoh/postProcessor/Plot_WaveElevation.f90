...
!   This is used for plotting purposes. It will compute coordinates needed to plot the wave
!   elevation.
!
!   @author TCSASSEMBLER
!   @version 1.1
SUBROUTINE Plot_WaveElevation(ID,Environment,iw,iBeta,RAOS,Results)
    ...
    DO i=1,Nx
        DO j=1,Ny
            ...
            IF (r.GT.0) THEN
                Potential=SQRT(kwave/(2.*PI*r))*CIH(kwave,0.,Environment%Depth)*&
                    &CEXP(II*(kwave*r-0.25*PI))*HKochin
            ELSE
                Potential=0.
            END IF
            ! -II factor because of the free surface definition (Real(-II*A*exp(II*(kx-wt)))
            EtaP(i,j)=-II*1./Environment%G*II*w*Potential
            Eta(i,j)=EtaI(i,j)+EtaP(i,j)
        END DO
    END DO
    OPEN(10,FILE=ID%ID(1:ID%lID)//'/results/WaveField.tec')
    WRITE(10,'(A)') 'VARIABLES="X" "Y" "etaI_C" "etaI_S" &
    &"etaP_C" "etaC_S" "etaI_C+etaP_C" "etaI_S+etaI_P" "|etaP|" "|etaI+etaP|"'
    WRITE(10,'(A,E14.7,A,I6,A,I6,A)') 'ZONE t="Wave frequency - w =',&
        &w,'",N=',Nx*Ny,', E=',(Nx-1)*(Nx-1),' , F=FEPOINT,ET=QUADRILATERAL'
    DO i=1,Nx
        DO j=1,Ny
            WRITE(10,'(10(1X,E14.7))') X(i),Y(j),REAL(etaI(i,j)),AIMAG(etaI(i,j)),&
                &REAL(etaP(i,j)),AIMAG(etaP(i,j)),REAL(etaI(i,j)+etaP(i,j)),&
                &AIMAG(etaI(i,j)+etaP(i,j)),ABS(etaP(i,j)),ABS(eta(i,j))
        END DO
    END DO
    DO i=1,Nx-1
        DO j=1,Ny-1
