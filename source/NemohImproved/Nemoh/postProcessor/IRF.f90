...
!   Contains utilites for creating, deleting, initializing an IRF
!
!   @author TCSASSEMBLER
!   @version 1.1
MODULE MIRF
...
CONTAINS
    ...
    SUBROUTINE CreateTIRF(IRF,Ntime,Nradiation,Nintegration)
        ...
        ALLOCATE(IRF%Time(0:Ntime-1),IRF%AddedMass(Nradiation,Nintegration),&
            &IRF%K(0:Ntime-1,Nradiation,Nintegration))
            ...
            DO j=1,Results%Nradiation
                DO k=1,Results%Nintegration
                    ...
                    DO l=1,Results%Nw-1
                        ...
                        IRF%K(i,j,k)=IRF%K(i,j,k)-0.5*(Results%RadiationDamping(l,j,k)*&
                            &COS(Results%w(l)*IRF%Time(i))+Results%RadiationDamping(l+1,j,k)*&
                            &COS(Results%w(l+1)*IRF%Time(i)))*(Results%w(l+1)-Results%w(l))
                    END DO 
            ...
            DO k=1,Results%Nintegration
                ...
                DO l=1,Results%Nw
                    ...
                    DO i=0,IRF%Ntime-2
                        ...
                        CM(l)=CM(l)+0.5*(IRF%K(i,j,k)*SIN(Results%w(l)*IRF%Time(i))+&
                            &IRF%K(i+1,j,k)*SIN(Results%w(l)*IRF%Time(i+1)))*(IRF%Time(i+1)-IRF%Time(i))
                    END DO  
    ...
    SUBROUTINE Save_IRF(IRF,namefile)
        ...
        DO j=1,IRF%Nradiation
            ...
            DO i=0,IRF%Ntime-1                
                WRITE(10,'(80(1X,E14.7))') IRF%Time(i),(IRF%AddedMass(j,k),IRF%K(i,j,k),k=1,IRF%Nintegration)
            END DO
        END DO
        CLOSE(10)
